...
export const getPackage = async (packageName: string) => {
  ...
  const palDependencies = Object.keys(packageJson.dependencies || {}).filter(
    (dep) => dep.startsWith('@repo') && dep !== '@repo/shadcn-ui'
  );

  const dependencies = Object.keys(packageJson.dependencies || {}).filter(
    (dep) =>
      !['react', 'react-dom', '@repo/shadcn-ui', ...palDependencies].includes(
        dep
      )
  );
  ...
  const fileContents = await Promise.all(
    tsxFiles.map(async (file) => {
      ...
      return {
        ...
        path: file.name,
        content,
        target: `components/pal-ui/${packageName}/${file.name}`,
      };
    })
  );
  ...
  for (const dep of palDependencies) {
    const packageName = dep.replace('@repo/', '');

    registryDependencies.push(
      `https://pal-ui.beamitpal.com/registry/${packageName}.json`
    );
  }
